    ...
    return(
        <div>
                ...
                <div className="mt-3 relative" ref={inputRef}>
                    <div className={`bg-gray50 px-4 py-[14px] rounded-[12px] border-[3px] border-gray50 flex justify-between cursor-pointer ${ focus && '!border-black300' }`} onClick={() => setFocus(!focus)}>
                        ...
                        <div className="shadow-appBar absolute top-[62px] bg-white w-full rounded-[12px] p-2 z-10">
                            {
                                options.length > 0 && options.map((option: string) => {
                                    return(
                                    <div className="p-[14px] flex gap-3 items-center cursor-pointer w-full" onClick={() =>  selectedOptions.includes(option) ?  setSelectedOptions(prev => prev.filter( currentOption => currentOption !== option )) : setSelectedOptions(prev => [...prev,option])  }>
                                        {
